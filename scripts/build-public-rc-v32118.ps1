...
if (-not $releaseWindow -or $releaseWindow.url -ne "payload/CELE_Topnotcher_OS_v3_21_0.html") {
  throw "RC4 requires direct startup into the reviewed CELE v3.21.0 payload."
}
...
if ($stagedManifest.files.Count -ne 105) { throw "Patched staged payload file count is not 105." }
...
Write-Host "Release-tool staging verified: 105/105 v3.21.18 payload files; compiler remains frozen."
...
$releaseIpcReport = Join-Path $OutputDirectory "rc4-release-webview-ipc-report.json"
...
try {
  ...
  if (-not $ipcProcess.HasExited) {
    ...
    throw "RC4 public-config WebView2/Tauri IPC startup timed out."
  }
} finally {
...
if (-not (Test-Path -LiteralPath $releaseIpcReport -PathType Leaf)) {
  throw "RC4 public-config startup did not produce an IPC certification report."
}
$ipcReport = Get-Content -Raw -LiteralPath $releaseIpcReport | ConvertFrom-Json
if ($ipcReport.status -ne "PASS") {
  throw "RC4 public-config WebView2/Tauri IPC certification is not PASS."
}
$ipcChecks = @($ipcReport.checks)
if ($ipcChecks.Count -ne 13 -or @($ipcChecks | Where-Object { $_.status -ne "PASS" }).Count -ne 0) {
  throw "RC4 public-config IPC report did not contain 13 passing checks."
}
Write-Host "RC4 public startup routing + WebView2/Tauri IPC: PASS (13/13)."
...
$rcExe = Join-Path $binariesDir "CELE-Topnotcher-OS-v3.21.18-RC4-unsigned.exe"
$rcInstaller = Join-Path $binariesDir "CELE-Topnotcher-OS-v3.21.18-RC4-unsigned-setup.exe"
...
$manifestOut = [ordered]@{
  schemaVersion = 1
  phase = "v3.21.18-ocr-startup-rootfix-rc4"
...
$readme = @"
CELE Topnotcher OS v3.21.18 RC4
...
CELE frontend interaction layer patched in v3.21.18; frozen compiler and CELE data/analytics/engineering semantics unchanged.
...
releaseReady: false
"@
$readme | Set-Content -LiteralPath (Join-Path $OutputDirectory "README-RC4.txt") -Encoding UTF8
