...
if (-not $releaseWindow -or $releaseWindow.url -ne "payload/CELE_Topnotcher_OS_v3_21_0.html") {
  throw "RC2 requires direct startup into the reviewed CELE v3.21.0 payload."
}
...
$releaseIpcReport = Join-Path $OutputDirectory "rc2-release-webview-ipc-report.json"
...
try {
  ...
  if (-not $ipcProcess.HasExited) {
    ...
    throw "RC2 public-config WebView2/Tauri IPC startup timed out."
  }
} finally {
...
if (-not (Test-Path -LiteralPath $releaseIpcReport -PathType Leaf)) {
  throw "RC2 public-config startup did not produce an IPC certification report."
}
$ipcReport = Get-Content -Raw -LiteralPath $releaseIpcReport | ConvertFrom-Json
if ($ipcReport.status -ne "PASS") {
  throw "RC2 public-config WebView2/Tauri IPC certification is not PASS."
}
$ipcChecks = @($ipcReport.checks)
if ($ipcChecks.Count -ne 13 -or @($ipcChecks | Where-Object { $_.status -ne "PASS" }).Count -ne 0) {
  throw "RC2 public-config IPC report did not contain 13 passing checks."
}
Write-Host "RC2 public startup routing + WebView2/Tauri IPC: PASS (13/13)."
...
$rcExe = Join-Path $binariesDir "CELE-Topnotcher-OS-v3.21.12-RC2-unsigned.exe"
$rcInstaller = Join-Path $binariesDir "CELE-Topnotcher-OS-v3.21.12-RC2-unsigned-setup.exe"
...
$manifestOut = [ordered]@{
  schemaVersion = 1
  phase = "v3.21.12-public-startup-routing-rc2"
...
$readme = @"
CELE Topnotcher OS v3.21.12 RC2
...
releaseReady: false
"@
$readme | Set-Content -LiteralPath (Join-Path $OutputDirectory "README-RC2.txt") -Encoding UTF8
