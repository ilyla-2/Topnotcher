...
const result=await page.evaluate(async()=>{
 ...
 const wrappers={current:typeof window.nativeOcrCurrentScanPage==='function',all:typeof window.nativeOcrAllScanPages==='function',api:!!window.CELE_OCR_UI_WIRE_V32118};
...
await browser.close();
const pass=result.groups.every(g=>result.originalHandlers[g])&&result.resilience?.pass&&result.wrappers.current&&result.wrappers.all&&result.wrappers.api&&result.noFileClick&&pageErrors.length===0;
