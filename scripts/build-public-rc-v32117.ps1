...
if (-not $releaseWindow -or $releaseWindow.url -ne "payload/CELE_Topnotcher_OS_v3_21_0.html") {
  throw "RC3 requires direct startup into the reviewed CELE v3.21.0 payload."
}
...
if ($stagedManifest.files.Count -ne 104) { throw "Patched staged payload file count is not 104." }
...
Write-Host "Release-tool staging verified: 104/104 v3.21.17 payload files; compiler remains frozen."
...
$releaseIpcReport = Join-Path $OutputDirectory "rc3-release-webview-ipc-report.json"
...
try {
  ...
  if (-not $ipcProcess.HasExited) {
    ...
    throw "RC3 public-config WebView2/Tauri IPC startup timed out."
  }
} finally {
...
if (-not (Test-Path -LiteralPath $releaseIpcReport -PathType Leaf)) {
  throw "RC3 public-config startup did not produce an IPC certification report."
}
$ipcReport = Get-Content -Raw -LiteralPath $releaseIpcReport | ConvertFrom-Json
if ($ipcReport.status -ne "PASS") {
  throw "RC3 public-config WebView2/Tauri IPC certification is not PASS."
}
$ipcChecks = @($ipcReport.checks)
if ($ipcChecks.Count -ne 13 -or @($ipcChecks | Where-Object { $_.status -ne "PASS" }).Count -ne 0) {
  throw "RC3 public-config IPC report did not contain 13 passing checks."
}
Write-Host "RC3 public startup routing + WebView2/Tauri IPC: PASS (13/13)."
...
$rcExe = Join-Path $binariesDir "CELE-Topnotcher-OS-v3.21.17-RC3-unsigned.exe"
$rcInstaller = Join-Path $binariesDir "CELE-Topnotcher-OS-v3.21.17-RC3-unsigned-setup.exe"
...
$manifestOut = [ordered]@{
  schemaVersion = 1
  phase = "v3.21.17-ui-interaction-resilience-rc3"
...
$readme = @"
CELE Topnotcher OS v3.21.17 RC3
...
CELE frontend interaction layer patched in v3.21.17; frozen compiler and CELE data/analytics/engineering semantics unchanged.
...
releaseReady: false
"@
$readme | Set-Content -LiteralPath (Join-Path $OutputDirectory "README-RC3.txt") -Encoding UTF8
