param(
  ...
  [string]$OutputDirectory = ".\CELE-v3.21.15-manual-desktop-ui-evidence",
...
$ExpectedInstallerSha256 = "6d5b378d3c46850c6f55e1046c9880f6fe8fa6b030bc23fc2d4cacddaa5d7d3b"
...
if($installerSha -ne $ExpectedInstallerSha256){
  throw "Installer SHA-256 mismatch. Expected exact RC2 $ExpectedInstallerSha256, found $installerSha"
}
$signature=Get-AuthenticodeSignature -LiteralPath $InstallerPath
if($signature.Status -ne [System.Management.Automation.SignatureStatus]::NotSigned){
  throw "This kit expects exact unsigned RC2. Signature status is $($signature.Status)."
}

$invalidPdf=Join-Path $fixtures "invalid-not-a-real-pdf.pdf"
"This is deliberately not a PDF. CELE v3.21.15 rejection fixture." | Set-Content -LiteralPath $invalidPdf -Encoding ASCII

$entry=Get-AppEntry
if(-not $entry -or [string]$entry.DisplayVersion -ne $ExpectedVersion){
  Write-Host "Installing exact RC2 silently..."
  $p=Start-Process -FilePath $InstallerPath -ArgumentList "/S" -Wait -PassThru
  if($p.ExitCode -ne 0){throw "RC2 installation failed with exit code $($p.ExitCode)."}
  $entry=$null
...
if([string]$entry.DisplayVersion -ne $ExpectedVersion){
  throw "Expected installed RC2 version $ExpectedVersion, found $($entry.DisplayVersion)."
}
...
$overall = if($pickerPass -and $dpiPass){"COMPLETE_PASS_CANDIDATE"}else{"COMPLETE_WITH_FAILURES"}

$report=[ordered]@{
  schemaVersion=1
  phase="v3.21.15-manual-desktop-ui-certification"
  status=$overall
...
$reportPath=Join-Path $OutputDirectory "v3.21.15-manual-desktop-ui-report.json"
